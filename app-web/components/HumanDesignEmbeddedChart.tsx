...
export default function HumanDesignEmbeddedChart({
  ...
  useEffect(() => {
    ...
    fetch('/human-design-chart.svg')
      ...
      .then(svgText => {
        console.log('✓ 成功加载用户原始SVG，长度:', svgText.length)
        setSvgContent(svgText)
      })
      .catch(error => {
        console.error('❌ 加载用户SVG失败:', error)
        // 如果用户的SVG也加载失败，使用简化的备用SVG
        const fallbackSvg = `<svg width="750" height="1240" viewBox="0 0 750 1240" xmlns="http://www.w3.org/2000/svg">
          <rect width="750" height="1240" fill="#f0f0f0"/>
          <text x="375" y="620" text-anchor="middle" font-size="24" fill="#333">人类图加载失败</text>
        </svg>`
      ...
      <div className={`human-design-embedded-chart ${className}`}>
        <div className="flex items-center justify-center" style={{ width, height }}>
          <div className="text-center">
            ...
            <p className="text-gray-300">加载用户原始人类图中...</p>
          </div>
        </div>
      </div>
    )
